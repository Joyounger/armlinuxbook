 ...
 */
ChameleonMScreen::ChameleonMScreen(QWidget *parent, const char* name) : QWidget(parent)
{
    setWhatsThis(QString(name));
    vm_stopped = false;
...
void ChameleonMScreen::init() {
   ...
   */
  qpixmap=qpixmap.scaled(SCREEN_WIDTH, SCREEN_HEIGHT);
...
{    
    if (newSize == fullScreenSize) {
        ...
        qpixmap=qpixmap.scaled(getDisplayFullWidth(), getDisplayFullHeight());
    } else {
        qpixmap=qpixmap.scaled(getDisplayWidth(), getDisplayHeight());
    }
...
void ChameleonMScreen::setNextVMTimeSlice(int millis) {
    if (millis < 0) {
        ...
        if (vm_slicer.isActive()) {
            vm_slicer.setInterval(millis);
        } else {
	    vm_slicer.setSingleShot(true);
...
void ChameleonMScreen::refresh(int x1, int y1, int x2, int y2) {
	...
	painter.drawPixmap(x1 + SCREEN_X, y1 + SCREEN_Y, qpixmap, x1, y1, (x2 - x1 + 1), (y2 - y1 + 1));
    }
    force_refresh = true;
    last_pen = last_brush = -1;
}
