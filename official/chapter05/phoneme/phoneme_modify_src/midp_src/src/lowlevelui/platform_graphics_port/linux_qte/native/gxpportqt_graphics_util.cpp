...
get_transformed_pixmap(QPixmap* originalPixmap,
	...
	originalImage.setAlphaChannel(originalImage);
    }
    ...
    if (0 != transform) {
	QImage sectionImage32bpp = sectionImage.convertToFormat(QImage::Format_RGB32);
	...
	/* Qt specific */
	processedImage=processedImage.scaled(t_width, t_height);
	...
	if(TRUE == sectionImage.hasAlphaChannel() ) {
	    processedImage.setAlphaChannel(processedImage);
	} else {
...
void transform_image(QImage* srcQImage, int transform) {
    ...
    /* Qt specific */
    QImage processedQImage = QImage(t_width, t_height, QImage::Format_RGB32);
    ...
    /* ---------- */
    if (srcQImage->hasAlphaChannel() == true) 
