/****************************************************************************
** Meta object code from reading C++ file 'mixer.h'
**
** Created: Tue Jun 2 10:47:48 2009
**      by: The Qt Meta Object Compiler version 59 (Qt 4.3.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../mixer.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'mixer.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 59
#error "This file was generated using the moc from 4.3.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

static const uint qt_meta_data_Mixer[] = {

 // content:
       1,       // revision
       0,       // classname
       0,    0, // classinfo
       2,   10, // methods
       0,    0, // properties
       0,    0, // enums/sets

 // slots: signature, parameters, type, tag, flags
       7,    6,    6,    6, 0x0a,
      31,    6,    6,    6, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_Mixer[] = {
    "Mixer\0\0on_QuitButton_clicked()\0"
    "setMasterVolume(int)\0"
};

const QMetaObject Mixer::staticMetaObject = {
    { &MixerBase::staticMetaObject, qt_meta_stringdata_Mixer,
      qt_meta_data_Mixer, 0 }
};

const QMetaObject *Mixer::metaObject() const
{
    return &staticMetaObject;
}

void *Mixer::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Mixer))
	return static_cast<void*>(const_cast< Mixer*>(this));
    return MixerBase::qt_metacast(_clname);
}

int Mixer::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = MixerBase::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: on_QuitButton_clicked(); break;
        case 1: setMasterVolume((*reinterpret_cast< int(*)>(_a[1]))); break;
        }
        _id -= 2;
    }
    return _id;
}
